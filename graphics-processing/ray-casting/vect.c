...
PUBLIC Vect vShrink(Vect v, double s)
{
	v.x /= s;
	v.y /= s;
	v.z /= s;
...
PUBLIC Vect vDiv(Vect a, Vect b) { return (vInit(a.x/b.x, a.y/b.y, a.z/b.z)); }

PUBLIC double vLen2(Vect v) { return (vDot(v, v)); }

PUBLIC double vLen(Vect v) { return (sqrt(vLen2(v))); }

PUBLIC Vect vUnit(Vect v) { return (vShrink(v, vLen(v))); }
