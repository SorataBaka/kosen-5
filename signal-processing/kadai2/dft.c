...
void bit_reversal_permutation(Cx *x, int N)
{
	int j = 0;
	for (int i = 1; i < N; i++)
	{
...
void FFT(Cx *x, Cx *X, int N)
{
	bit_reversal_permutation(x, N);
	for (int len = 2; len <= N; len <<= 1)
	{
		double angle = -2.0 * M_PI / len;
		Cx wlen = cos(angle) + I * sin(angle);
...
int main(int argc, char *argv[])
{
	...
	int N = 512;		 // 分析対象区間の標本数  --> Number of samples in the analysis interval
