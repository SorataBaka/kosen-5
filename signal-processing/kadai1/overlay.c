...
int main(int argc, char *argv[])
{
  ...
  for (int i = 1; i < fileCount - 1; i++)
  {
    ...
    {
      double leftTemp = newWav->val[0][newWavIndex] + wav->val[0][readFileIndex];
      if (leftTemp > 32767)
        leftTemp = 32767;
      else if (leftTemp < -32768)
        leftTemp = -32768;
      newWav->val[0][newWavIndex] = leftTemp;

      if (channelCount >= 2 && wav->fmt.ch >= 2)
      {
        double rightTemp = newWav->val[1][newWavIndex] + wav->val[1][readFileIndex];
        if (rightTemp > 32767)
          rightTemp = 32767;
        else if (rightTemp < -32768)
          rightTemp = -32768;
        newWav->val[1][newWavIndex] = rightTemp;
      }
      newWavIndex++;
    }
    fclose(fp);
