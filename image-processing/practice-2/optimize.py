...
def variance_ratio(image, start=1, stop=255):
    ...
    for i in range(start, stop):
        ...
        var_between = prob0 * (class0_mean - global_mean)**2 + prob1 * (class1_mean - global_mean)**2
    ...
    for _ in range(max_iter):
        ...
        for y in range(h):
            for x in range(w):
                ...
                classification = np.abs(pixel_value - center0) < np.abs(pixel_value - center1)
                if classification:
        ...
        if abs(new_center0 - center0) < epsilon and abs(new_center1 - center1) < epsilon:
            center0 = new_center0
            center1 = new_center1
            break
        center0 = new_center0
...
parser.add_argument("--algorithm", help="Select threshold optimizing algorithm to use.", choices=["dispersion", "kmeans"], required=True)
